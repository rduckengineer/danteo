...
namespace {
struct TestStates {
    static constexpr danteo::engine::State first{"first"sv};
    static constexpr danteo::engine::State second{"second"sv};
    static constexpr danteo::engine::State third{"third"sv};
};

struct TestEvents {
    static constexpr danteo::engine::Event next{"next"sv};
    static constexpr danteo::engine::Event end{"end"sv};
};
} // namespace
...
SCENARIO("The navigation graph represents the graph of allowed state changes") {
    GIVEN("A navigation graph") {
        auto builder = danteo::engine::StateGraph::Builder{};

        builder / TestStates::second + TestEvents::next = TestStates::third;

        danteo::engine::StateGraph navigation = std::move(builder).build();
