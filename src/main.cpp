...
// configuration step. It creates a namespace called `danteo`. You can modify
...
struct Bitmap : ftxui::Node {
    ...
    void ComputeRequirement() override {
        requirement_ = ftxui::Requirement{.min_x = static_cast<int>(width_),
    ...
    void Render(ftxui::Screen& screen) override {
        for (std::size_t x = 0; x < width_; ++x) {
            for (std::size_t y = 0; y < height_ / 2; ++y) {
                auto& p = screen.PixelAt(box_.x_min + static_cast<int>(x),
                                         box_.y_min + static_cast<int>(y));
                p.character = "▄";
...
void game_iteration_canvas() {
    ...
    auto game_iteration = [&](const std::chrono::steady_clock::duration elapsed_time) {
        ...
        // this isn't actually timing based for now, it's just updating the display however fast it
        // can
        fps = 1.0 /
              (static_cast<double>(
        ...
        auto& small_bm_pixel = small_bm->data().at(static_cast<std::size_t>(elapsed_time.count()) %
                                                   small_bm->data().size());
...
int main(int argc, const char** argv) {
 ...
 Options:
        ...
        std::map<std::string, docopt::value> args = docopt::docopt(
            ...
            fmt::format("{} {}", danteo::cmake::project_name,
                        danteo::cmake::project_version)); // version string, acquired
